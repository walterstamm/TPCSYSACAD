...
SELECT * FROM Alumnos
insert into Alumnos(Cuil, Apellido, Nombre, FechaNacimiento, Email, Domicilio, Localidad)
values(20214539048, 'GOMEZ', 'HUGO FABIAN', '03-01-1970', 'LOCO@HOTMAIL.COM', 'PERON 374', 'SAN FERNANDO')
use TPCSYSACAD
select * from usuariodocente
select * from Docentes

insert into Docentes(Cuil, Apellido, Nombre, Nacionalidad, FechaNacimiento,         Email,   Domicilio, UsuarioEstado, Estado)
values(                20,  'gomez', 'hugo',        'arg',       '03-1-1970', 'jdjd@jfj.zq', 'peron 555',             0,     1)
use tpcsysacad
select ud.* from Docentes d left join UsuarioDocente ud on d.IdDocente=ud.IdDocente 
left join Usuarios u on ud.Usuario=u.Id

select * from Usuarios
insert into Usuarios(Nombre) values('admin')
update usuarios set Nombre = 'DOC'
use tpcsysacad
select ud.id, ud.usuario, ud.password, u.nombre, ud.tipousuario, ud.iddocente from UsuarioDocente ud inner join Usuarios u on ud.TipoUsuario=u.id
where ud.Usuario = @usuario and ud.Password = @password

select ud.id, ud.usuario, ud.password, u.nombre, ud.tipousuario, ud.iddocente from UsuarioDocente ud inner join Usuarios u on ud.TipoUsuario=u.id

insert into UsuarioDocente(Usuario, Password, TipoUsuario, IdDocente)
values(1, 121345, 1, 1)
update UsuarioDocente SET Usuario = 'DOC'  
GO
SELECT IDDOCENTE, CUIL, APELLIDO, NOMBRE, NACIONALIDAD, FECHANACIMIENTO, EMAIL, DOMICILIO, USUARIOESTADO, ESTADO 
from DOCENTES
